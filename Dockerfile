...
# ── Multi-stage build for the leti-server binary ──────────────────
...
COPY . .
RUN cargo build --release --bin leti-server \
    && strip target/release/leti-server
...
RUN apt-get update \
    ...
    && rm -rf /var/lib/apt/lists/* \
    && useradd --system --create-home --uid 10001 leti

COPY --from=builder /app/target/release/leti-server /usr/local/bin/leti-server

# Data root is a volume so sqlite/artifacts/sessions persist across
# container restarts. Owned by the non-root runtime user.
ENV LETI_DATA_DIR=/data
RUN mkdir -p /data && chown leti:leti /data
VOLUME ["/data"]
...
# container is gated by LETI_ALLOW_NON_LOOPBACK at the app layer.
ENV LETI_BIND=0.0.0.0:8787 \
    LETI_ALLOW_NON_LOOPBACK=1 \
    LETI_LOG_FORMAT=json \
    RUST_LOG=info

EXPOSE 8787
# Optional metrics port (Phase 10). Only served when LETI_METRICS_BIND
