 ...
 *
 * REFERENCE SNAPSHOT ONLY — this file is NOT imported by any runtime code.
 * The hand-rolled DTOs in `types.ts` are the types the client actually uses.
 * This snapshot is regenerated by `npm run codegen` and diffed by the CI
 * `contract-drift` job to catch when the server's OpenAPI schema drifts from
 * the hand-rolled `types.ts`. Keeping it committed is what lets that job run
