...
into the bind defaults. Cloud / multi-tenant deployment is a host concern;
see [`cloud-integration.md`](cloud-integration.md) for the integration
contract.

## Process model

`leti-server` is a single binary. It owns its sqlite file, its artifact
...
The TUI (`leti`, npm) is a separate process that talks to the server
...
|---|---|---|---|
| `LETI_BIND` | `127.0.0.1:8787` | no | TCP bind. Loopback only by default. |
| `LETI_DATA_DIR` | `~/.leti` | no | Sqlite + artifacts + session logs root. |
| `OPENAI_API_KEY` | _(unset)_ | yes (for live model) | Bearer token for OpenRouter. |
| `LETI_DEFAULT_MODEL` | `anthropic/claude-sonnet-4-6` | no | Default chat model id. |
...
| `LETI_MAX_COST_USD` | `5.00` | no | Per-session hard limit. |
| `LETI_LOG_FORMAT` | `json` | no | `json` or `pretty`. |
| `LETI_METRICS_BIND` | _(unset)_ | no | If set (e.g. `127.0.0.1:9464`), serves Prometheus metrics at `GET /metrics` on that address. Unset → metrics fully off. |
| `LETI_WORKSPACE` | `<data_dir>/workspace` | no | Default agent workspace root. |
...
for support / forensics and is the input to `leti-server audit`.

## Bind safety

The default `127.0.0.1:8787` is enforced by the loaded `Config`, not by
any iptables rule. To expose to LAN, set `LETI_BIND` explicitly. There
...
Detached mode is explicit and opt-in. The reference server can make newly
created sessions detached:

```bash
leti-server serve --detached --on-ask deny
leti-server serve --detached --on-ask allow
```

`--on-ask allow` only resolves ordinary Ask decisions. Ruleset Deny always
wins; destructive shell operations and `web_fetch` still require explicit
permission rules. Every detached permission check is written as a durable
authorization event. Turns injected by background or sibling delivery remain
fail-closed. Top-level detached turns are not automatically resumed after a
process restart; use the persisted transcript and an explicit new turn.
...
# then: OPENAI_API_BASE_URL=<printed base_url> cargo run -p leti-server
```

The mock returns canned `simple_text` by default; embed
`PARITY_SCENARIO:<name>` in the user message text to pick another
scenario (see `crates/leti-test-mock-provider/src/scenarios.rs`).

## Resource limits

- `LETI_MAX_COST_USD` caps the per-session spend.
...
into a log shipper. Switch to `LETI_LOG_FORMAT=pretty` for local
...
leti-ai is software, not an infra bundle: **running it locally needs no
...
endpoint only start when `LETI_METRICS_BIND` is set.

```bash
# No metrics — plain local run.
cargo run -p leti-server -- serve

# Opt in: serve Prometheus metrics on a separate loopback port.
LETI_METRICS_BIND=127.0.0.1:9464 cargo run -p leti-server -- serve
# then point a Prometheus scraper at http://127.0.0.1:9464/metrics
```

Exposed metrics: `leti_turns_total`, `leti_tokens_total{kind}`,
`leti_cost_usd_total`, `leti_provider_retries_total`,
`leti_tool_executions_total{tool,outcome}`, `leti_compactions_total`,
`leti_plugin_faults_total{hook}`.
