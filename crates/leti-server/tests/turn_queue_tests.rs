...
use leti_server::app_state::{TurnHandle, TurnOrigin};
use leti_server::injected_turn::{UNTRUSTED_SYSTEM_CLAUSE, enqueue_or_start_turn, wrap_untrusted};
...
async fn seed_session(state: &leti_server::AppState) -> SessionId {
...
async fn user_text_bodies(state: &leti_server::AppState, sid: SessionId) -> Vec<String> {
    ...
    for m in msgs {
        ...
        for p in parts {
            if let leti_core::types::part::Part::Text { text, .. } = p {
                out.push(text);
            }
        }
    }
