...
use leti_server::AuthPrincipal;
use leti_server::middleware::{WORKSPACE_HEADER, WorkspaceRoutingLayer};
use leti_server::workspace_resolver::{WorkspaceError, WorkspaceResolver};
use leti_server::{AppState, StaticWorkspaceResolver};
...
/// re-export (`leti_server::AuthPrincipal`) — the SAME path
/// `main.rs` and the question route use.
#[tokio::test]
async fn one_canonical_principal_satisfies_layer_and_guard() {
    async fn guarded(
        Extension(_state): Extension<Arc<AppState>>,
        Extension(principal): Extension<leti_server::AuthPrincipal>,
    ) -> impl IntoResponse {
    ...
    let app = Router::new()
        ...
        .layer(from_fn(
            |mut req: Request<Body>, next: axum::middleware::Next| async move {
                ...
                req.extensions_mut()
                    .insert(leti_server::AuthPrincipal::user("canon"));
                next.run(req).await
            },
        ));
    let resp = app
