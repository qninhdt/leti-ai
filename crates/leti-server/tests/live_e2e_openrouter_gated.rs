...
//! provider only when `LETI_LIVE_E2E=1` AND `OPENAI_API_KEY` are set;
...
//!     cargo test -p leti-server --test live_e2e_openrouter_gated
//!
//! Cost guardrails: the cheapest model the caller selects via
//! `LETI_LIVE_E2E_MODEL` (default `openai/gpt-4o-mini`), `max_tokens`
