...
fn stub_memory() -> Arc<dyn leti_core::adapters::memory_store::MemoryStore> {
    ...
    impl MemoryStore for NoopMemory {
        ...
        ) -> Result<leti_core::types::message::MessageId, MemoryError> {
            Ok(msg.id)
        }
...
fn stub_spawner() -> Arc<dyn leti_core::tools::builtins::subagent_task::SubagentSpawner> {
    ...
    use leti_core::tools::builtins::subagent_task::SpawnedSubagent;

    struct StubSpawner;

    #[async_trait]
    impl leti_core::tools::builtins::subagent_task::SubagentSpawner for StubSpawner {
        async fn spawn(
            &self,
            _ctx: &ToolCtx,
            _subagent_type: &str,
