...
use leti_core::tools::ToolScheduler;
...
use leti_plugin_api::dispatch::HookChains;
...
pub enum TurnOrigin {
    ...
    BackgroundTaskSettled {
        task_id: leti_core::runtime::subagent::TaskId,
...
/// workspace. Host applications can inject other port implementations
...
pub struct AppState {
    ...
    /// Default session interaction policy for the reference daemon. Cloud
    /// hosts may choose a different policy per request in their own routes.
    pub default_interaction_mode: leti_core::types::session::InteractionMode,
    ...
    /// `LETI_SUBAGENT_MAX_PER_SESSION` (default 32) per ROOT session.
...
impl AppState {
    ...
    pub async fn require_session(
        &self,
        sid: SessionId,
    ) -> Result<leti_core::types::session::SessionMeta, crate::error::AppError> {
        self.memory
            .get_session(sid)
            .await?
    ...
    pub async fn try_cancel_active_turn(&self, sid: SessionId) -> bool {
        ...
        crate::events::publish_status(
            &self.events,
            sid,
            leti_core::types::session::SessionStatus::Cancelling,
        )
        .await;
        true
    }
