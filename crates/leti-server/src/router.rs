...
impl RouterBuilder {
    ...
    pub fn with_session_routes(mut self) -> Self {
        self.inner = self
            ...
            .routes(routes!(session::cancel_subagent))
            .routes(routes!(session::interrupt_subagent))
    ...
    /// `LETI_CORS_ALLOW_ORIGINS` to a comma-separated origin list
    /// (e.g. `https://app.example.com,https://admin.example.com`) to
    /// allow cross-origin browsers; set `LETI_CORS_PERMISSIVE=1` for
    ...
    pub fn build_with_auth(self, state: AppState, authenticator: Arc<dyn Authenticator>) -> Router {
        ...
        // Gate Swagger UI on LETI_ENABLE_DOCS. Defaults to ON for
        ...
        // LETI_ENABLE_DOCS=0 in cloud deploys.
        let docs_enabled = std::env::var("LETI_ENABLE_DOCS")
...
fn build_cors_layer() -> CorsLayer {
    if std::env::var("LETI_CORS_PERMISSIVE")
        .map(|v| v == "1" || v.eq_ignore_ascii_case("true"))
        .unwrap_or(false)
    {
        tracing::warn!(
            "LETI_CORS_PERMISSIVE=1 — CORS layer accepts any origin; \
             do not enable in production"
        );
        return CorsLayer::permissive();
    }

    if let Ok(origins) = std::env::var("LETI_CORS_ALLOW_ORIGINS") {
