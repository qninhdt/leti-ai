...
/// boot from `LETI_SHELL_IMPL` so a production regression in the emulated
...
/// Parse the `LETI_SHELL_IMPL` value into a [`ShellImpl`]. Pure so it can be
...
fn parse_shell_impl(raw: Option<&str>) -> Result<ShellImpl, String> {
    match raw.map(str::trim).filter(|s| !s.is_empty()) {
        ...
        Some(v) => match v.to_ascii_lowercase().as_str() {
            ...
            other => Err(format!(
                "LETI_SHELL_IMPL='{other}' is not recognized (expected 'emulated' or 'subprocess')"
            )),
        },
    }
}

/// Resolve the shell implementation from the raw environment value.
fn resolve_shell_impl(raw: Option<&str>) -> Result<ShellImpl, String> {
    parse_shell_impl(raw)
}

/// Pre-built adapters shared by both the serve and doctor paths.
pub struct AdapterStack {
    pub artifacts: Arc<dyn leti_core::adapters::ArtifactStore>,
    pub provider: Arc<dyn leti_core::adapters::ModelProvider>,
    pub memory: Arc<dyn leti_core::adapters::MemoryStore>,
    pub events: Arc<dyn leti_core::adapters::EventSink>,
    pub workspace_root: PathBuf,
    /// Injected local filesystem implementation.
    pub fs: Arc<dyn Filesystem>,
    pub shell: Arc<dyn leti_core::tools::builtins::bash::ShellExecutor>,
    ...
    pub python: Arc<dyn leti_core::tools::builtins::python::PythonExecutor>,
    ...
    pub web_fetcher: Arc<dyn leti_core::tools::builtins::web_fetch::WebFetcher>,
}
...
impl AdapterStack {
    ...
    pub async fn build(opts: AdapterStackConfig<'_>) -> anyhow::Result<Self> {
        ...
        let pool = leti_adapters::sqlite::open_pool(&db_path, opts.pool_size)
            .await
        ...
        let event_repo = leti_adapters::sqlite::event_repo::SqliteEventRepo::new(pool.clone());
        ...
        // file ops route through `ctx.fs`. Host applications can provide
        // another Filesystem implementation at their composition root.
        let fs: Arc<dyn Filesystem> = Arc::new(LocalFilesystem::new(workspace_root.clone()));
        ...
        // the legacy `LocalShellExecutor`. Read the raw value with
        // `var_os` so a non-UTF-8 value surfaces as a parse error rather than
        // silently defaulting (`var().ok()` would swallow `NotUnicode`).
        let shell_impl_raw = std::env::var_os("LETI_SHELL_IMPL");
        let shell_impl_str = match shell_impl_raw.as_ref() {
            Some(os) => Some(
                os.to_str()
                    .ok_or_else(|| anyhow::anyhow!("LETI_SHELL_IMPL contains non-UTF-8 bytes"))?,
            ),
            None => None,
        };
        let shell_impl = resolve_shell_impl(shell_impl_str).map_err(|e| anyhow::anyhow!(e))?;
        let shell: Arc<dyn leti_core::tools::builtins::bash::ShellExecutor> = match shell_impl {
            ShellImpl::Emulated => Arc::new(EmulatedShellExecutor::new()),
            ShellImpl::Subprocess => {
                tracing::warn!(
                    "LETI_SHELL_IMPL=subprocess: using the legacy subprocess bash executor \
        ...
        let web_fetcher: Arc<dyn leti_core::tools::builtins::web_fetch::WebFetcher> =
            Arc::new(leti_adapters::webfetch::ReqwestWebFetcher::new());
...
mod tests {
    ...
    // --- resolve_shell_impl: the actual build() wiring decision, proven
    // without opening a sqlite pool or mutating env.

    #[test]
    fn resolve_defaults_to_emulated_in_local_mode() {
        // Unset env + local FS → emulated (the production default).
        assert_eq!(resolve_shell_impl(None).unwrap(), ShellImpl::Emulated);
    }

    #[test]
    fn resolve_subprocess_selected_in_local_mode() {
        // The rollback lever: LETI_SHELL_IMPL=subprocess in local mode
        // resolves to the subprocess executor (build() then constructs
        // LocalShellExecutor for this variant).
        assert_eq!(
            resolve_shell_impl(Some("subprocess")).unwrap(),
            ShellImpl::Subprocess
        );
    }
    ...
    fn resolve_propagates_parse_error() {
        ...
        assert!(resolve_shell_impl(Some("nope")).is_err());
    }
}
