...
use leti_core::types::permission::{AlwaysScope, AskId, PermissionAction};
use leti_protocol::{PermissionReplyDto, PermissionReplyKind};
use uuid::Uuid;
...
pub async fn reply(
    ...
    let resolved_decision = match body.decision {
        PermissionReplyKind::Allow | PermissionReplyKind::AlwaysAllow => {
            leti_core::types::permission::Decision::Allow
        }
        PermissionReplyKind::Deny | PermissionReplyKind::AlwaysDeny => {
            leti_core::types::permission::Decision::Deny {
...
fn map_perm_err(e: leti_core::error::PermissionError) -> AppError {
