...
use leti_core::runtime::PRESERVE_RECENT;
use leti_core::types::event::AgentEvent;
use leti_core::types::message::{Message, MessageId, Role};
use leti_core::types::part::Part;
use leti_core::types::session::{SessionId, SessionStatus};
use leti_protocol::{CompactAckDto, CreateMessageDto, MessageDto, PartDto, PromptAckDto};
...
async fn drive_compaction(
    ...
    agent_id: leti_core::types::agent::AgentId,
    cancel: CancellationToken,
) -> Result<(), leti_core::error::CoreError> {
...
async fn drive_loop(
    ...
    let mut setup = crate::turn_driver::build_loop_context(&state, session_id, agent_id).await?;
    if let leti_core::types::session::InteractionMode::Detached { on_ask } =
        setup.loop_ctx.interaction_mode
    {
        let permission = setup.loop_ctx.handles.permission.clone();
        setup.loop_ctx.handles.permission = Arc::new(
            crate::detached_permission::DetachedPermissionManager::new(permission, on_ask),
        );
    }
    state
        .runtime
