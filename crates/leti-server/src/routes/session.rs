...
use leti_core::runtime::subagent::{BackgroundTransition, TaskId};
...
use leti_core::types::session::{SessionCapabilities, SessionFilter, SessionId, SessionMeta};
use leti_protocol::ContinueSubagentDto;
use leti_protocol::{
...
pub async fn create(
    ...
    let mut meta = meta;
    meta.interaction_mode = body
        .interaction_mode
        .unwrap_or(state.default_interaction_mode);
...
pub async fn delete(
    ...
    publish_status(
        &state.events,
        sid,
        leti_core::types::session::SessionStatus::Cancelled,
    )
    .await;
