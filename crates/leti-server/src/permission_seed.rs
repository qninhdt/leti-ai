...
use leti_core::types::permission::{PermissionAction, PermissionRule};
...
///
/// `LETI_PERMISSIVE_TOOLS=0` disables the seed entirely (every op then
...
pub fn default_permission_rules() -> Vec<PermissionRule> {
    let seed_enabled = std::env::var("LETI_PERMISSIVE_TOOLS")
