...
pub struct ServeArgs {
    ...
    /// `Config` already reads `LETI_BIND`; this flag wins over both.
    #[arg(long)]
    pub bind: Option<String>,
    /// Start new sessions in detached mode. This is opt-in and defaults off.
    #[arg(long)]
    pub detached: bool,
    /// Decision for ordinary Ask outcomes in detached sessions.
    #[arg(long, value_enum, default_value_t = DetachedOnAsk::Deny)]
    pub on_ask: DetachedOnAsk,
}

#[derive(Debug, Clone, Copy, clap::ValueEnum)]
pub enum DetachedOnAsk {
    Allow,
    Deny,
}
...
impl Cli {
    ...
    pub fn resolved_command(self) -> Command {
        self.command.unwrap_or(Command::Serve(ServeArgs {
            bind: None,
            detached: false,
            on_ask: DetachedOnAsk::Deny,
        }))
    }
}
