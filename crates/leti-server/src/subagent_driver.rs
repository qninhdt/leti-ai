...
use leti_core::adapters::memory_store::SubagentExecutionPatch;
use leti_core::error::CoreError;
use leti_core::projection::ProjectionCaps;
use leti_core::runtime::LoopContext;
...
pub(crate) async fn drive_subagent(
    ...
    agent_slug: leti_core::agent::AgentSlug,
    child_perm: Arc<dyn leti_core::adapters::permission_manager::PermissionManager>,
    ...
    handle_name: leti_core::runtime::subagent::HandleName,
    parent_ext: leti_core::runtime::TurnExtensions,
) -> Result<(), CoreError> {
    ...
    let execution = state
        ...
        .await?
        .ok_or_else(|| {
            CoreError::Memory(leti_core::error::MemoryError::Io(
    ...
    let child_perm = match child_meta.as_ref().map(|m| m.interaction_mode) {
        Some(leti_core::types::session::InteractionMode::Detached { on_ask }) => Arc::new(
            crate::detached_permission::DetachedPermissionManager::new(child_perm, on_ask),
        )
            as Arc<dyn leti_core::adapters::permission_manager::PermissionManager>,
        _ => child_perm,
    };
    let handles =
    ...
    let loop_ctx = LoopContext {
        ...
        mode: child_meta
            ...
            .unwrap_or_default(),
        interaction_mode: child_meta
            .as_ref()
            .map(|m| m.interaction_mode)
            .unwrap_or_default(),
    ...
    if !recovered_inbox.is_empty() {
        ...
        state
            ...
            .await?;
        let messages = leti_core::runtime::prepare_session_messages(
            &loop_ctx.handles,
            child_session_id,
    ...
    loop {
        ...
        match leti_core::runtime::prepare_session_messages(
            &loop_ctx.handles,
            child_session_id,
            ProjectionCaps::default(),
            leti_core::runtime::ReminderRequestContext::default(),
...
pub(crate) async fn publish_roster(state: &AppState, root: SessionId) {
    let entries = state
        ...
        .into_iter()
        .map(
            |(name, task_id, generation)| leti_core::types::event::RosterFrameEntry {
                name: name.to_string(),
                task_id: task_id.0,
                generation,
            },
