//! `OpenAiProvider::list_models` — catalog fetch + decode.
//!
...
async fn list_models_decodes_catalog_from_mock_service() {
    let mock = leti_test_mock_provider::MockOpenAiService::spawn()
        .await
        .expect("spawn mock");
    let provider = OpenAiProvider::new(mock.base_url(), Some(SecretString::from("test-key")));
...
async fn list_models_maps_401_to_auth_error() {
    ...
    let provider = OpenAiProvider::new(server.uri(), Some(SecretString::from("bad")));
...
async fn list_models_maps_500_to_network_error() {
    ...
    let provider = OpenAiProvider::new(server.uri(), Some(SecretString::from("test-key")));
    let err = provider.list_models().await.expect_err("must fail");
