...
use leti_adapters::sqlite::permission_repo::SqlitePermissionRepo;
...
async fn submit_ask(
    ...
    session: SessionId,
) -> leti_core::types::permission::AskId {
...
async fn persisted_rule_recovered_on_reload_after_crash_before_inmemory_push() {
    ...
    let repo = SqlitePermissionRepo::new(pool.clone());
    let rec = leti_adapters::sqlite::permission_repo::PermissionRecord {
        session_id: session,
        ask_id: leti_core::types::permission::AskId::new(),
        permission: "test:perm".into(),
        decision: leti_adapters::sqlite::permission_repo::PersistedDecision::Always,
    };
