...
use leti_core::adapters::filesystem::{Filesystem, WriteOpts};
use tokio_util::sync::CancellationToken;

use super::ast::{AndOr, AndOrOp, Command, Node, Script, Word};
use super::builtins::{self, BuiltinCtx};
use super::env::ShellEnv;
use super::error::fs_err_msg;
...
fn push_capped(buf: &mut String, truncated: &mut bool, s: &str, cap: usize) {
    ...
    if s.len() <= room {
        ...
        let cut = crate::util::floor_char_boundary(s, room.min(s.len()));
        buf.push_str(&s[..cut]);
        *truncated = true;
    }
}
