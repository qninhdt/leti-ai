...
use leti_core::types::message::{Message, MessageId};
use leti_core::types::pagination::{Page, PageResult};
...
use super::codec::{
    decode_json, encode_json, interaction_mode_parts, map_io, mode_str, now_ms, part_kind,
    role_str, status_str,
};
...
impl MemoryStore for SqliteMemoryStore {
    ...
    async fn create_session(
        ...
        let (interaction_mode, detached_on_ask) =
            interaction_mode_parts(leti_core::types::session::InteractionMode::Interactive);
        ...
        sqlx::query(
            r#"INSERT INTO sessions
                (id, agent_id, parent_session_id, status, permission_mode,
                 version, created_at, updated_at, deleted_at, extensions,
                 capabilities, current_agent_slug, previous_agent_slug, depth, model,
                 interaction_mode, detached_on_ask)
    ...
    async fn create_session_with_meta(&self, meta: SessionMeta) -> Result<SessionId, MemoryError> {
        ...
        let (interaction_mode, detached_on_ask) = interaction_mode_parts(meta.interaction_mode);
    ...
    async fn list_sessions(&self, filter: SessionFilter) -> Result<Vec<SessionMeta>, MemoryError> {
        let mut sql = String::from(
            "SELECT id, agent_id, parent_session_id, status, permission_mode, \
             ...
             current_agent_slug, previous_agent_slug, depth, model, interaction_mode, detached_on_ask \
    ...
    async fn append_background_task_settled(
        ...
        .bind(message_id.to_string())
        .bind(settlement.parent_session_id.to_string())
        .bind(role_str(leti_core::types::message::Role::User))
        .bind(settlement.parent_session_id.to_string())
    ...
    async fn record_observation(
        &self,
        obs: leti_core::adapters::memory_store::ReadObservation,
    ) -> Result<(), MemoryError> {
    ...
    async fn list_observations(
        &self,
        session: SessionId,
    ) -> Result<Vec<leti_core::adapters::memory_store::ReadObservation>, MemoryError> {
