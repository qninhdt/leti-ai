...
use leti_core::runtime::question_registry::QuestionId;
...
use leti_core::types::permission::{AskId, Decision, PermissionRequest};
use leti_core::types::session::{SessionId, SessionStatus};
use leti_protocol::EventDto;
...
fn subagent_roster_converts() {
    let ev = AgentEvent::SubagentRoster {
        root_session_id: sid(),
        entries: vec![leti_core::types::event::RosterFrameEntry {
            name: "reviewer".into(),
            task_id: Uuid::nil(),
            generation: 7,
        }],
...
fn todo_updated_converts() {
    let ev = AgentEvent::TodoUpdated {
        session_id: sid(),
        items: vec![
            leti_core::types::event::TodoEventItem {
