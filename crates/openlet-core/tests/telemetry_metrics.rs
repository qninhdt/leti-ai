...
use openlet_core::dispatch::{DispatchOutcome, FaultKind, PluginFault, publish_fault_if_any};
use openlet_core::error::EventError;
use openlet_core::hooks::HookKind;
...
fn plugin_fault_emits_counter_with_hook_label_and_no_workspace_label() {
    ...
    let fault = snapshot
        .iter()
        .find(|(ck, _, _, _)| ck.key().name() == "openlet_plugin_faults_total")
