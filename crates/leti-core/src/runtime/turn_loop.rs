...
use crate::types::session::InteractionMode;

use super::ConversationRuntime;
use super::TurnExtensions;
use super::TurnInput;
...
pub struct LoopContext {
    ...
    /// Opaque host data forwarded to every tool and permission check.
...
impl ConversationRuntime {
    ...
    pub async fn run_loop(
        ...
        loop {
            ...
            metrics::counter!("leti_turns_total").increment(1);
            ...
            if let Some(u) = outcome.usage.as_ref() {
                ...
                metrics::counter!("leti_tokens_total", "kind" => "prompt")
                    .increment(u.input_tokens);
                metrics::counter!("leti_tokens_total", "kind" => "completion")
                ...
                if let Some(cost) = outcome.cost_usd {
                    ...
                    if let Some(c) = cost.to_f64() {
                        ...
                        metrics::gauge!("leti_cost_usd_total").increment(c);
                    }
                }
            ...
            let perm_ctx = PermissionCtx {
                session_id,
                mode: loop_ctx.mode,
                interaction_mode: loop_ctx.interaction_mode,
                ext: loop_ctx.ext.clone(),
            };
