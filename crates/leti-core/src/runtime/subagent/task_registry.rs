...
impl TaskRegistry {
    ...
    /// small number; production reads `LETI_SUBAGENT_MAX_PER_SESSION`
    /// via [`Self::from_env`].
    #[must_use]
    ...
    /// Build a registry honoring the `LETI_SUBAGENT_MAX_PER_SESSION`
    /// and `LETI_SUBAGENT_MAX_LIFETIME_SPAWNS` env overrides (defaults
    /// [`DEFAULT_MAX_PER_SESSION`] / [`DEFAULT_MAX_LIFETIME_SPAWNS`]).
    #[must_use]
    pub fn from_env() -> Self {
        let max = std::env::var("LETI_SUBAGENT_MAX_PER_SESSION")
            .ok()
            .and_then(|v| v.parse::<usize>().ok())
            .unwrap_or(DEFAULT_MAX_PER_SESSION);
        let lifetime = std::env::var("LETI_SUBAGENT_MAX_LIFETIME_SPAWNS")
            .ok()
            .and_then(|v| v.parse::<usize>().ok())
