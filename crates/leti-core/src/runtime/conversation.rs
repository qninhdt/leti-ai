...
/// Runtime tunables. Plumbed from `leti_core::config::Config` at boot.
#[derive(Debug, Clone)]
pub struct RuntimeConfig {
    pub idle_timeout: Duration,
    pub default_model: String,
    /// Provider retry policy for transient failures (429 / 5xx) when
    /// opening a chat stream. See [`RetryConfig`].
...
impl ConversationRuntime {
    ...
    async fn drive_stream(
        ...
        let mut stream = self
            .chat_stream_with_retry(session_id, req, cancel.clone())
            .await?;
