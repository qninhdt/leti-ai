...
//! used to call `tokio::fs` directly. That made leti-ai-core
//! laptop-only by accident: there was no place to plug in alternate storage
//! implementations, a sandboxed FUSE mount, or a mocking layer for tests.
//!
//! `Filesystem` is the missing seam. Tools call `ctx.fs.*`; the impl
//! decides what "the workspace" actually is. The local impl
//! (`leti_adapters::localfs::LocalFilesystem`) keeps today's
//! tokio-fs behavior with the deepest-existing-ancestor canonicalize
//! trick. Other implementations map the same calls onto their own storage
//! service.
//!
