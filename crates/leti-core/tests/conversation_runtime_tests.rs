...
use leti_core::error::{CoreError, EventError, MemoryError, ProviderError};
use leti_core::projection::{LlmMessage, LlmRole};
use leti_core::runtime::{ConversationRuntime, RuntimeConfig, TurnInput};
use leti_core::types::agent::AgentId;
use leti_core::types::event::{AgentEvent, DeltaKind, EventFilter, Usage};
...
struct MockEventSink {
    tx: broadcast::Sender<leti_core::adapters::event_sink::DeliveredEvent>,
...
impl EventSink for MockEventSink {
    async fn publish(&self, ev: AgentEvent, p: Persistence) -> Result<(), EventError> {
        ...
        let _ = self
            .tx
            .send(leti_core::adapters::event_sink::DeliveredEvent {
                event_id: None,
                event: ev,
            });
        Ok(())
