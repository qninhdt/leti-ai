...
use leti_core::permission::{Deferred, DeferredSender, deferred_pair};
use leti_core::tools::{
    PromptPolicy, ReadHistory, Tool, ToolConcurrency, ToolDispatchResult, ToolInvocation,
...
use leti_core::types::session::{DetachedAsk, InteractionMode, SessionId};
...
impl leti_core::adapters::memory_store::MemoryStore for NoopMemory {
    ...
    ) -> Result<MessageId, leti_core::error::MemoryError> {
        Ok(msg.id)
    }
...
async fn unsafe_write_is_an_ordered_wave_barrier() {
    ...
    impl Tool for CountingRead {
        ...
        fn concurrency(&self, input: &Self::Input) -> ToolConcurrency {
...
#[tokio::test]
async fn detached_tool_execution_emits_durable_authorization_event() {
    ...
    let dir = TempDir::new().unwrap();
    let workspace = dir.path().to_path_buf();
    let interaction_mode = InteractionMode::Detached {
    ...
    let results = dispatch_batch(
        ...
        perm_ctx,
        vec![ToolInvocation {
            call_id: "detached-audit".into(),
            name: "echo".into(),
            args: json!({ "tag": "audited" }),
        }],
    )
    .await;

    assert!(results[0].outcome.is_ok());
    assert!(recording.snapshot().iter().any(|(event, persistence)| {
        matches!(
            (event, persistence),
            (
                ...
                Persistence::Durable
            ) if *event_session == session_id && *mode == interaction_mode
        )
    }));
}
