...
use leti_core::adapters::event_sink::{DeliveredEvent, EventSink, Persistence};
...
use leti_core::error::{ArtifactError, EventError, MemoryError, PermissionError};
use leti_core::tools::ReadHistory;
use leti_core::tools::Tool;
use leti_core::tools::builtins::plan_mode::{
    EnterPlanModeInput, ExitPlanModeInput, PLAN_AGENT_SLUG,
};
use leti_core::tools::builtins::{EnterPlanModeTool, ExitPlanModeTool};
...
fn ctx_with_bus(workspace: &std::path::Path, sid: SessionId, bus: Arc<RecordingBus>) -> ToolCtx {
    use leti_core::runtime::QuestionRegistry;
...
fn seed_meta(sid: SessionId, current: Option<&str>, previous: Option<&str>) -> SessionMeta {
    SessionMeta {
        ...
        capabilities: leti_core::types::session::SessionCapabilities::default(),
...
async fn allowlist_enforced_at_dispatch() {
    ...
    use leti_core::runtime::agent_allowlist::{partition_by_allowlist, resolve_allowlist};
    use leti_core::tools::ToolInvocation;
    ...
    match &denied[0].2 {
        leti_core::error::ToolError::NotAllowedInAgent { tool, agent } => {
