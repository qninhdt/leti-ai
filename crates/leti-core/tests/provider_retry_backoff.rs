...
use leti_core::adapters::{EventSink, MemoryStore, ModelProvider};
use leti_core::error::ProviderError;
use leti_core::runtime::{ConversationRuntime, RetryConfig, RuntimeConfig, TurnInput};
...
fn turn(session_id: SessionId) -> TurnInput {
    TurnInput {
        session_id,
        messages: vec![leti_core::projection::LlmMessage {
            role: leti_core::projection::LlmRole::User,
            content: "hi".into(),
            reasoning: None,
            tool_calls: vec![],
            tool_call_id: None,
...
async fn rate_limit_then_success_retries_and_completes() {
    ...
    assert_eq!(
        ...
        leti_core::adapters::model_provider::FinishReason::EndTurn
    );
...
async fn auth_error_is_not_retried() {
    ...
    assert!(
        matches!(
            err,
            leti_core::error::CoreError::Provider(ProviderError::Auth(_))
        ),
...
async fn exhausts_attempts_then_surfaces_last_error() {
    ...
    assert!(
        matches!(
            err,
            leti_core::error::CoreError::Provider(ProviderError::RateLimit { .. })
        ),
